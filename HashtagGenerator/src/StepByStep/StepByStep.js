      ...
      {shouldShowAllSteps ? (
        <table className="table-step">
          ...
          <tbody>
            ...
            />
            <StepRow
              description="2. Get Facebook Page access token"
              method="GET"
              endpoint={`${facebookPages[0]?.id}`}
              requestQueryParams={{
                access_token: facebookUserAccessToken,
              ...
              onResponseReceived={(response) => {
                setFacebookPageAccessToken(response.access_token);
              }}
              isDisabled={!facebookUserAccessToken}
            />
            <StepRow
              description="2. Get Facebook Page and POST to it"
              method="POST"
              endpoint={`${facebookPages[0]?.id}/feed`}
              requestQueryParams={{
                access_token: facebookPageAccessToken,
                link: "https://img.freepik.com/free-photo/beef-cotlet-burger-with-sauce-wooden-board_114579-2600.jpg",
                message: "Look at this awesome #burger",
              }}
              onResponseReceived={() => {}}
              isDisabled={!facebookUserAccessToken && !facebookPageAccessToken}
            />
          </tbody>
        </table>
      ) : null}
